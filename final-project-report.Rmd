...
data_clean_combine = data_clean_combine[,order(colnames(data_clean_combine))]
#Imput missing data
data_clean_delete_missing <- data_clean_combine[!(data_clean_combine$TEMPF == -9 |
                                                    data_clean_combine$TEMPDF == -9 |
                                                    data_clean_combine$TOTDIAG == -9 |
                                                    data_clean_combine$TOTPROC == -9 |
                                                    data_clean_combine$WAITTIME == -9 |
                                                    data_clean_combine$WAITTIME == -7), ]
#Delete all the char variables, will not work in the glm_fit
...
data_clean_delete_var <- data_clean_delete_char %>% 
  select(-c(AGER, AGEDAYS, 
            ...
            HOSPCODE, LBTC, LWBS, MSA, PATCODE, PATWT, RACERFL, RESPRD, PTONLINEE1: PTONLINEE6,
            RFID, SEXFL, TRANNH, TRANOTH, TRANPSYC, YEAR))


#Number of variables left
ncol(data_clean_delete_var)
#Type of each column -> need to change into factor for some
sapply(data_clean_delete_var, typeof)
#Find the means for each column for imputing
means <- apply(data_clean_delete_var, 2, function(x) {
  mean(x[x>0])
  })
...
# drop columns where values are all the same
data_clean_delete = data_clean_delete[vapply(data_clean_delete, function(x) length(unique(x)) > 1, logical(1L))]
...
data_clean_delete = data_clean_delete[,order(colnames(data_clean_delete))]

#data_clean_labeled = data_clean_delete
#data_clean_labeled$col_types = 
#data_clean_labeled %>% type_convert(col_types ="fff-f-fi-fffffiffffffff----ffffiffiiiiffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffff--ffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffff-fffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffff---------------fffff-fffffffffffffffffffffff-fiif-fffffffffffffffff-ffffffff-iiiffffffffffifffffffffi--fffffffffffffiffffffffffffffffffffffffffffffffffffffffffiiff-fffffi-ff--fffifddiifif---fffffffifff-f")
```
