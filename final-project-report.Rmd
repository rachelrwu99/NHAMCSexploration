...
sapply(data_delete_var, typeof)


#Find the means for each column for imputing
#LOV replace -7, -9 and PAINSCALE replace -8, POPCT, BPDIAS,BPSYS
means <- apply(data_delete_var, 2, function(x) {
  mean(x[x>0])
  })

#View(data_delete_var)
data_delete_var


# drop columns where values are all the same
data_clean_delete = data_delete_var[vapply(data_delete_var, function(x) length(unique(x)) > 1, logical(1L))]
