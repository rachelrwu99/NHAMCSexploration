...
```
**Around 10.6% of people were admitted to the hospital.**
```{r}
# correlation matrix
...
mean(pred_tree != admit_test$ADMITHOS)
```
...
mean(rf_predictions != admit_test$ADMITHOS)
```
...
plot(gbm_fit_optimal, i.var = "CBC", n.trees = optimal_num_trees, type = "response")
```
...
mean(gbm_predictions != admit_test$ADMITHOS)
```
