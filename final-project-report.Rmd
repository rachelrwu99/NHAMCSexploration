...
data_factored$AGE[is.na(data_factored$AGE) | data_factored$AGE < 0] <- means[1]
data_factored$BPDIAS[is.na(data_factored$BPDIAS) | data_factored$BPDIAS < 2] <- means[2]
data_factored$BPSYS[is.na(data_factored$BPSYS) | data_factored$BPSYS < 2] <- means[3]
...
```{r}
glm_fit = glm(ADMITHOS ~ RACEUN + ARREMS + PAYTYPER + ABG ,
              family = "binomial",
              data = admit_train)
# glm_fit = glm(ADMITHOS ~ .,
#               family = "binomial",
#               data = admit_train)
#sapply(lapply(data_factored, unique), length)
#sapply(data_factored, typeof)
coef(glm_fit)
